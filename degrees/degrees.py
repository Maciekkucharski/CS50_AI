...
def shortest_path(source, target):
    ...
    stack_frontier.add(source_node)
    while True:
        #check if frontier empty
        if stack_frontier.empty():
            return None
        
        #remove value from frontier
        node = stack_frontier.remove()
        #check if node is goal
        if node.person == target_node.person:
            movies = []
            people = []
        ...
        #mark as explored
        explored_set.add(node.person)
        #add neghbors to frontier
        neighbors = neighbors_for_person(node.person)
        for neighbor in neighbors:
            if not stack_frontier.contains_state(neighbor[1]) and neighbor[1] not in explored_set:
                child = Node(person=neighbor[1], movie=neighbor[0], parent=node)
